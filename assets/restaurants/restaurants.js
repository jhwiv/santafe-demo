...
// Build a Google Maps URL for a restaurant or any place-like object.
// Prefers the exact address (most accurate pin), falls back to name + city.
// Opens the Google Maps app on iOS if installed, otherwise google.com/maps.
export function googleMapsUrlFor(place) {
  if (!place) return null;
  let query;
  if (place.address && String(place.address).trim()) {
    query = String(place.address).trim();
  } else if (place.name) {
    query = place.city ? `${place.name}, ${place.city}` : place.name;
  } else {
    return null;
  }
...
class PickerState {
  constructor(dataset, baseKey) {
    ...
    this.keys = {
      ...
      times:    `${baseKey}-times`,    // { 'YYYY-MM-DD': 'HH:MM' } per-night reservation time override
    };
    ...
    this.times    = this._loadObj(this.keys.times);
  ...
  resetAll() {
    ...
    this._save(this.keys.times, this.times);
    if (typeof localStorage !== 'undefined') {
  ...
  // Per-night reservation time override (HH:MM, 24-hour). Falls through to
  // night.time → party.defaultTime when unset. Used by booking links, .ics
  // export, and the condensed view dinner row.
  timeFor(night)            {
    const override = this.times[night.date];
    if (override) return override;
    const fromDataset = night?.time ? toHHMM(night.time) : null;
    return fromDataset || toHHMM(this.party?.defaultTime || '19:00');
  }
  setTime(night, hhmm)      {
    if (hhmm) this.times[night.date] = hhmm; else delete this.times[night.date];
    this._save(this.keys.times, this.times);
    this._notify();
  }
...
function downloadIcs(restaurant, night, state, booking) {
  const time = state.timeFor(night);
...
function renderBrowseChips(night, state, placement) {
  ...
  if (night) {
    otParams.set('dateTime', `${night.date}T${state.timeFor(night)}:00`);
  }
...
function renderCustomForm(night, state, rerender, editing) {
  ...
  const urlLabel = el('label', { class: 'tr-custom-form-label', for: 'tr-cf-url-' + night.date }, 'Website (optional)');
  const urlInput = el('input', {
    id: 'tr-cf-url-' + night.date,
    type: 'text', inputmode: 'url',
    class: 'tr-custom-form-input',
    placeholder: 'e.g. pasquals.com',
    autocomplete: 'off',
  });
  if (editing?.url) urlInput.value = editing.url;
  grid.append(el('div', { class: 'tr-custom-form-field tr-cf-field-url' }, urlLabel, urlInput));

  // Address (optional) — when present the picked card gets a precise
  // Google Maps link pinned to this address instead of a name+city search.
  const addrLabel = el('label', { class: 'tr-custom-form-label', for: 'tr-cf-addr-' + night.date }, 'Address (optional)');
  const addrInput = el('input', {
    id: 'tr-cf-addr-' + night.date,
    type: 'text', class: 'tr-custom-form-input',
    placeholder: 'e.g. 1501 Paseo de Peralta, Santa Fe, NM',
    autocomplete: 'street-address',
  });
  if (editing?.address) addrInput.value = editing.address;
  grid.append(el('div', { class: 'tr-custom-form-field tr-cf-field-addr' }, addrLabel, addrInput));
  ...
  saveBtn.addEventListener('click', (e) => {
    ...
    // Auto-prepend https:// if the user typed a bare domain like 'pasquals.com'.
    if (url && !/^https?:\/\//i.test(url) && /^[\w.-]+\.[a-z]{2,}/i.test(url)) {
      url = 'https://' + url;
      urlInput.value = url;
    }
    if (url && !isValidHttpUrl(url)) { showErr('Please enter a valid website (e.g. pasquals.com).', urlInput); return; }
...
function renderCustomCard(night, state, custom, rerender) {
  ...
  // Google Maps link for custom restaurant — use address if present, else
  // search the restaurant name in Santa Fe.
  const customMaps = googleMapsUrlFor({
    address: custom.address,
    name: custom.name,
    city: 'Santa Fe',
  });

  // Per-night reservation time editor (custom restaurants get the same
  // control as curated picks so users can override the stored custom.time).
  card.append(renderTimeEditor(night, state, rerender));

  // Actions: edit / remove / maps
  const actions = el('div', { class: 'tr-actions tr-custom-actions' });
  if (customMaps) {
    actions.append(el('a', { class: 'tr-btn-link', href: customMaps, target: '_blank', rel: 'noopener' }, 'Open in Google Maps ↗'));
  }
...
// Inline reservation-time editor. Shows the current effective time and lets
// the user tap to change it. Saves to state.times[night.date]. Used by both
// picked (curated) and custom restaurant cards.
function renderTimeEditor(night, state, rerender, opts = {}) {
  const wrap = el('div', { class: 'tr-time-edit' });
  const currentHHMM = state.timeFor(night);
  const label = el('span', { class: 'tr-time-edit-label' }, 'Reservation time:');
  const display = el('span', { class: 'tr-time-edit-value' }, formatTimeLabel(currentHHMM));
  // Native time input — best mobile UX (iOS wheel picker).
  const input = el('input', {
    type: 'time',
    class: 'tr-time-edit-input',
    value: currentHHMM,
    step: '300', // 5-min increments
    'aria-label': 'Reservation time for this night',
  });
  input.addEventListener('change', () => {
    const v = input.value;
    if (!v || !/^\d{2}:\d{2}$/.test(v)) return;
    state.setTime(night, v);
    if (opts.onChange) opts.onChange(v);
    if (rerender) rerender();
  });
  // Visually hide the native input but keep it tappable. The display span
  // shows the formatted label; tapping anywhere on the wrap opens the
  // native picker via .click() on the input.
  wrap.append(label, display, input);
...
function renderBackupCard(dataset, r, night, state, rerender, pickedR) {
  ...
  const backupMaps = googleMapsUrlFor(r);
  if (backupMaps) {
    actions.append(el('a', { class: 'tr-btn-link', href: backupMaps, target: '_blank', rel: 'noopener' }, 'Open in Google Maps ↗'));
  }
  card.append(actions);
...
function renderPickedCard(dataset, r, night, state, rerender) {
  ...
  if (r.phone) actions.append(el('a', { class: 'tr-btn-secondary', href: 'tel:' + r.phone }, formatPhone(r.phone)));
  if (r.website) actions.append(el('a', { class: 'tr-btn-link', href: r.website, target: '_blank', rel: 'noopener' }, 'Website ↗'));
  // Google Maps link — prefer the exact address, fall back to name + city.
  const mapsHref = googleMapsUrlFor(r);
  if (mapsHref) {
    actions.append(el('a', { class: 'tr-btn-link', href: mapsHref, target: '_blank', rel: 'noopener' }, 'Open in Google Maps ↗'));
  }
  card.append(actions);

  // Per-night reservation time editor
  card.append(renderTimeEditor(night, state, rerender));
  ...
  // (real-world example: traveler had Geronimo booked on Mon but the confirmation was
...
function renderBookingButton(r, night, state) {
  ...
  // Apply per-night time override (state.timeFor) so OpenTable etc.
  // pre-fill the user's chosen reservation time.
  const nightWithTime = { ...night, time: state.timeFor(night) };
  const url = buildBookingUrl(r, nightWithTime, state.party) || b.url;
